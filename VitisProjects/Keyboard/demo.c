...
static volatile  int RxDataRecv = FALSE; /* Flag to indicate Receive Data */
static volatile  int RxError = FALSE; 	/* Flag to indicate Receive Error */
static volatile  int RxOverFlow = FALSE; /* Flag to indicate Receive Overflow */
static volatile  int TxDataSent = FALSE; /* Flag to indicate Tx Data sent */
static volatile  int TxNoAck = FALSE;    /* Flag to indicate Tx No Ack */
static volatile  int TimeOut = FALSE;    /* Flag to indicate Watchdog Timeout */
static volatile  int TxNumBytes = 0;	/* Number of bytes transmitted */
static volatile  int RxNumBytes = 0;     /* Number of bytes received */
...
int main(void){
	...
	//Enable the Receive interrupts and the global interrupt in the PS/2 device.
	axi_ps2_IntrEnable(&Ps2Inst, axi_ps2_IPIXR_RX_ALL);
	axi_ps2_IntrGlobalEnable(&Ps2Inst);

	xil_printf("\r\nPS/2 Keyboard Demo using Interrupt Mode\r\n");

	//Initialize the flags that are set in the Interrupt Callback function.
	TxDataSent = FALSE;
	TxNumBytes = 0;
...
u8 KeyboardInit (void){
	...
	if(KbWaitReply(Ps2Ack)) KeyInst.KbInitStage++;
	else return 0;
...
//This function waits for the PS/2 keyboard to reply with a certain code, given as parameter.
u8 KbWaitReply(u8 Reply){
	while (RxDataRecv == FALSE) {;}
	if (KeyInst.Key != Reply) return 0;
	else return 1;
}
...
static int Ps2SetupIntrSystem(XIntc* IntcInstPtr, axi_ps2 *Ps2Ptr, u16 IntrId ){
	 ...
	 */
	Status = XIntc_Connect(IntcInstPtr,IntrId,(XInterruptHandler) axi_ps2_IntrHandler,Ps2Ptr);
	if (Status != XST_SUCCESS) return XST_FAILURE;
	...
	XIntc_Enable(IntcInstPtr, IntrId);//Enable the interrupt for the Ps2 device. enables processor interrupts
	Xil_ExceptionInit();//Initialize the exception table.
	Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_INT,(Xil_ExceptionHandler) XIntc_InterruptHandler,IntcInstPtr);//Register the interrupt controller handler with the exception table.
	Xil_ExceptionEnable();  //Enable the exceptions.
